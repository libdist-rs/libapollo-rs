...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let cli_network = net::futures_manager::Protocol::<Transaction, ClientMsg>::new(config.id, config.num_nodes as types::Replica, config.root_cert.clone(), config.my_cert.clone(), config.my_cert_key.clone());
    ...
    // Setup networking
    let protocol_network = net::futures_manager::Protocol::<ProtocolMsg, ProtocolMsg>::new(config.id, config.num_nodes as types::Replica, config.root_cert.clone(), config.my_cert.clone(), config.my_cert_key.clone());
