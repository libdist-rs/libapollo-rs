...
use config::Node;
use std::error::Error;
use types::synchs::{ClientMsg, Transaction, ProtocolMsg};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let client_network = net::tokio_manager::Protocol::<Transaction, ClientMsg>::new(config.id, config.num_nodes as types::Replica, config.root_cert.clone(), config.my_cert.clone(), config.my_cert_key.clone());
    ...
    // Setup networking
    let protocol_network = net::tokio_manager::Protocol::<ProtocolMsg, ProtocolMsg>::new(config.id, config.num_nodes as types::Replica, config.root_cert.clone(), config.my_cert.clone(), config.my_cert_key.clone());
